...
pub fn spawn_parallax_background(
    ...
    let bg_size = Vec2::new(320.0 * 4.0, 180.0 * 4.0);
...
// Moves background up
// When goes out of the bounds -> move it at the bottom of the screen
pub fn move_parallax_background(
    ...
    for (mut parallax_bg_transform, parallax_bg) in parallax_background_query.iter_mut() {
        parallax_bg_transform.translation +=
            parallax_direction * PARALLAX_SPEED * time.delta_seconds();

        if parallax_bg_transform.translation.y > primary_window.height() {
            parallax_bg_transform.translation.y = -parallax_bg.size.y / 2.0;
        }
    }
}
